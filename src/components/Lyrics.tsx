  ...
  if (!track) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-legato-text-secondary">재생 중인 음악 없음</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto lyrics-container">
        {!lyrics ? (
          <div className="flex flex-col items-center justify-center h-full p-6 text-center">
            <div className="w-12 h-12 border-3 border-legato-primary border-t-transparent rounded-full animate-spin mb-4" />
            <p className="text-legato-text-secondary">가사를 불러오는 중...</p>
          </div>
        ) : (
          <div className="p-6">
            {/* Lyrics Source */}
            <div className="flex items-center gap-2 mb-6">
              <span className="text-xs text-legato-text-tertiary uppercase tracking-wider">
                출처: {lyrics.source}
              </span>
              {lyrics.url && (
                <a
                  ...
                  className="text-xs text-legato-primary hover:underline"
                >
                  원본 보기
                </a>
              )}
                  ...
                  {paragraph.split("\n").map((line, lIndex) => (
                    <p
                      ...
                      className={`
                        ...
                        ${
                          line.startsWith("[")
                            ? "text-legato-text-tertiary text-sm uppercase tracking-wider mt-6"
                            : "text-legato-text-primary"
                        }
                      `}
                    >
                      {line || "\u00A0"}
            ...
            {/* Footer */}
            <div className="mt-8 pt-4 border-t border-legato-separator">
              <p className="text-sm text-legato-text-tertiary text-center">
                가사 제공: Genius
              </p>
            </div>
          </div>
